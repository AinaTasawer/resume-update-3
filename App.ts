...
function addLanguageField() {
    const languageSection = document.getElementById('language-section') as HTMLDivElement;
...
document.getElementById('resume-form')?.addEventListener('submit', function(event) {
    ...
    const languages = Array.from(document.getElementsByClassName('language') as HTMLCollectionOf<HTMLInputElement>)
        .map((input, index) => {
            const proficiencyInput = document.getElementsByClassName('language-proficiency')[index] as HTMLInputElement;
            return { language: input.value, proficiency: proficiencyInput?.value };
        });
    ...
    generatedResume.innerHTML = `
        <div class="resume-content">
            ...
            <h3>Languages</h3>
            <ul>${languages.map(lang => `<li>${lang.language} - ${lang.proficiency}</li>`).join('')}</ul>
        </div>
    `;
